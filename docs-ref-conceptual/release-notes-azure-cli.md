...
* Prise en charge du module complémentaire SGX « confcom »

### <a name="ams"></a>AMS

* Mise à jour du module avec l’API Azure Media Services 2020.
* `az ams account encryption` : Nouveau sous-groupe permettant d’afficher ou de définir le chiffrement pour le compte de service multimédia
* `az ams account storage set-authentication` : Nouvelle commande permettant de définir l’authentification pour le compte de stockage associé au compte de service multimédia
* `az ams account create (mi-system-assigned)` : Nouveau paramètre --mi-system-assigned pour account create permettant de définir l’identité managée du compte multimédia
* `az ams account mru set` : Cette commande ne fonctionne plus pour les comptes Media Services créés avec la version 2020-05-01 ou ultérieure de l’API.
* `az ams live-event create (stretch-mode, key-frame-interval, transcrip-lang, use-static-hostname, custom hostname)` : Ajout de nouvelles options de paramètre à la commande live-event create
* `az ams live-event standby` : Nouvelle commande permettant de placer l’événement en direct en mode veille
* `az ams transform create (videoanalysismode, audioanalysis mode)` : Nouvelles options de paramètre pour transform create

### <a name="app-service"></a>App Service

* `az webapp config ssl bind` : Possibilité d’avoir webapp et appservice plan dans des groupes de ressources différents. Mises à jour également apportées au texte de référence
* Correctif n° 8743 : az webapp deploy
* Résolution de bogue : Ajout de generateRandomAppNames.json à la configuration
* `az functionapp create` : Prise en charge de la création d’applications dotnet-isolated en préversion.
* Correctif n° 12150 : Prise en charge de l’ID de sous-réseau dans vnet-integration add
* `az functionapp create` : Suppression de l’indicateur de préversion de Node.js 14.

### <a name="arm"></a>ARM

* `az deployment group/sub/mg/tenant validate/create/what-if` : Prise en charge des fichiers Bicep
* `az bicep install` : Nouvelle commande permettant d’installer Bicep CLI
* `az bicep upgrade` : Nouvelle commande permettant de mettre à niveau de Bicep CLI
* `az bicep build` : Nouvelle commande permettant de générer des fichiers Bicep
* `az bicep version` : Nouvelle commande permettant d’afficher la version actuellement installée de Bicep CLI
* `az bicep list-versions` : Nouvelle commande permettant d’afficher les versions disponibles de Bicep CLI
* `az managedapp definition update` : Ajout d’une nouvelle commande permettant de mettre à jour la définition de managedapp

### <a name="backup"></a>Sauvegarde

* `az backup recoverypoint show-log-chain` : Ajout de l’heure de début/fin dans la sortie de la table show-log-chain
* Résolution de bogue : Activation de la restauration dans un autre emplacement pour les éléments protégés SQL/SAP HANA

### <a name="cdn"></a>CDN

* Prise en charge de CLI pour la référence SKU AFD

### <a name="compute"></a>Compute

* `az vm (extension) image list` : Amélioration de la robustesse
* `az vmss create` : Correction d’un problème de type de licence
* Mise à niveau vers la version d’API 2020-12-01
* `az vm create` : ajout de `--enable-hotpatching`

### <a name="cosmos-db"></a>Cosmos DB

* Mise à niveau vers la version 3.0.0 et prise en charge de NetworkAclBypass + mise à jour de Mongo ServerVersion + stratégie de sauvegarde

### <a name="extension"></a>Extension

* Prise en charge de la configuration de l’URL d’index d’extension

### <a name="iot-central"></a>IoT Central

* `az iot central app` : Implémentation de plusieurs correctifs S360
* `az iot central app update` : Suppression de la nécessité de vérifier etag lors de la mise à jour de l’application iotc existante.
* Modification de resourceType (IotApps) pour qu’il soit en casse mixte.

### <a name="key-vault"></a>Key Vault

* [CHANGEMENT CASSANT] `az keyvault role assignment/definition list` : `roleDefinitionName` doit être `roleName` dans la sortie de la commande
* [CHANGEMENT CASSANT] `id` remplacé par `jobId`, `azureStorageBlobContainerUri` remplacé par `folderUrl` dans la sortie de la commande de `az keyvault backup/restore`, `az keyvault key restore`

### <a name="network"></a>Réseau

* Passage de la version « 2020-07-01 » à « 2020-08-01 »
* `az network public-ip create` : Prise en charge de « --zone 1 2 3 » après « 2020-08-01 »
* `az network routeserver peering` : Renommage de `--vrouter-name` en `--routeserver`
* `az network express-route peering create` : Prise en charge d’une adresse IPv6
* `az network public-ip create` : Exposition d’un nouvel argument `--tier`

### <a name="openshift"></a>OpenShift

* Mise à jour de l’avertissement de dépréciation pour az openshift

### <a name="search"></a>Rechercher

* `az search` : Correction du guide d’assistance sur `--identity-type`.

### <a name="sql"></a>SQL

* Mise à jour des exemples az sql mi
* `az sql db/elastic-pool create/update` : Ajout de l’argument maintenance-configuration
* `az sql db replica create` : Ajout de l’argument --secondary-type

### <a name="storage"></a>Stockage

* [CHANGEMENT CASSANT] `az storage account file-service-properties` : Activation par défaut de la suppression de la stratégie de rétention avec 7 jours de rétention côté serveur
* Correctif n° 16872 : az storage blob nécessite désormais une connexion même si connection-string est fourni (2.19)
* Correctif n° 16959 : az storage copy plante : Erreur de validation en raison d’une variable locale « service » référencée avant affectation
* Correctif n° 14054 : L’objet « NoneType » n’a pas d’attribut « __Name__ »
* Correctif n° 16679 : `az storage blob download` échoue avec « Autorisation refusée » si le fichier de destination est un répertoire
* Mise à niveau de la version de l’API de stockage vers 2021-01-01
* Prise en charge de la version dans la stratégie de gestion du cycle de vie
* Prise en charge de la gestion de l’accès par clé partagée au compte de stockage
* `az storage account network-rule` : Règles d’accès aux ressources en disponibilité générale
* Prise en charge du chiffrement double pour l’étendue du chiffrement
* `az storage account blob-service-properties update` : Prise en charge de --change-feed-retention-days
* Prise en charge de la réécriture d’objets blob existants
...
* `az acr connected-registry install info` : Ajout d’une nouvelle clé `ACR_SYNC_TOKEN_NAME` avec la même valeur que `ACR_SYNC_TOKEN_USERNAME`. Un avertissement indiquant que la deuxième clé va être dépréciée s’affiche.
* `az acr connected-registry install renew-credentials` : Ajout d’une nouvelle clé `ACR_SYNC_TOKEN_NAME` avec la même valeur que `ACR_SYNC_TOKEN_USERNAME`. Un avertissement indiquant que la deuxième clé va être dépréciée s’affiche.

### <a name="aks"></a>AKS

* Ajout de liaisons d’arrêt/de démarrage de cluster managé
* `az aks check-acr` : Correction de la vérification de la version de Kubernetes

### <a name="apim"></a>APIM

* Disponibilité générale du groupe de commandes

### <a name="app-config"></a>App Config

* [CHANGEMENT CASSANT] `az appconfig feature filter add` : Ajout d’objets JSON comme valeurs de paramètre du filtre de fonctionnalités

### <a name="app-service"></a>App Service

* `az appservice ase/plan` : Prise en charge d’ASEv3
* Correctifs #16026 et #16118 pour az appservice plan
* Correctif #16509 : Prise en charge de la préférence du système d’exploitation
* Amélioration du comportement de appservice ase create-inbound-services pour ignorer les services DNS et prendre en charge DNS pour ASEv2
* `az webapp up/az webapp create` : Correction des erreurs NoneType
* `az webapp up/create` : Meilleure gestion des erreurs en cas de nom d’application avec un point
* Correctif #16681 : `az webapp config ssl import` : Correction du bogue provoquant des échecs sur les clouds nationaux

### <a name="arm"></a>ARM

* `az provider register` : Prise en charge de l’inscription du groupe d’administration

### <a name="backup"></a>Sauvegarde

* Ajout de la fonctionnalité CRR pour IaaSVM et d’autres commandes CRR
* `az backup protectable-item list` : Ajout de protectable-item-type comme argument facultatif

### <a name="botservice"></a>BotService

* `az bot create/update` : Ajout des fonctionnalités de chiffrement `--cmk-key-url` et `--encryption-off`
* `az bot update` : Renommage de l’argument Encryption-OFF en CMK-OFF et mise à jour de la version de l’API

### <a name="compute"></a>Calcul

* [CHANGEMENT CASSANT] vmss create : Renommage des valeurs du mode d’orchestration
* Nouveau groupe de commandes sshkey. Autorisation du référencement d’une ressource de clé SSH lors de la création d’une machine virtuelle
* `az disk create/update` : Ajout du paramètre `--enable-bursting` pour prendre en charge le bursting de disque

### <a name="extension"></a>Extension

* Prise en charge de la correspondance du préfixe de commande d’extension pour une installation dynamique

### <a name="hdinsight"></a>HDInsight

* `az hdinsight create` : Ajout d’un nouveau paramètre `--enable-compute-isolation` pour prendre en charge la création de cluster avec isolation de calcul.

### <a name="key-vault"></a>Key Vault

* `az keyvault key import` : Prise en charge du paramètre `--curve` pour l’importation de clés BYOK
* `az keyvault certificate download` : Correction d’un appel de méthode déprécié/supprimé
* `az keyvault create/update` : Suppression de l’étiquette d’aperçu pour `--enable-rbac-authorization`

### <a name="monitor"></a>Superviser

* `az monitor metrics alert create` : Correction de l’erreur « Ressource introuvable »

### <a name="netappfiles"></a>NetAppFiles

* `az netappfiles account ad add` : Ajout du paramètre `--security-operators`.
* `az netappfiles volume create` : Ajout du paramètre `--smb-continuously-available`.
* `az netappfiles volume create` : Ajout du paramètre `--smb-encryption`.
* `az netappfiles` : N’est plus en mode aperçu.

### <a name="network"></a>Réseau

* [CHANGEMENT CASSANT] `az network vrouter` : Ce groupe de commandes est déprécié, utilisez `az network routeserver`.
* `az network routeserver` : Ajout d’un nouveau groupe de commandes.
* `az network application-gateway create` : Ajout du paramètre `--ssl-profile-id`
* `az network application-gateway client-cert` : Gestion du certificat client approuvé de la passerelle applicative
* `az network application-gateway ssl-profile` : Gestion des profils SSL de la passerelle applicative
* Prise en charge des connexions de points de terminaison privés à DigitalTwins

### <a name="profile"></a>Profil

* `az login` : Lancement du navigateur dans WSL 2

### <a name="rdbms"></a>SGBDR

* `az mysql flexible-server create --iops` : Possibilité pour l’utilisateur de choisir IOPS pour sa référence SKU.
* Mise à jour de la commande de restauration Postgres pour prendre en charge la zone disponible
...
* Ajout de nouvelles commandes pour `az security`

### <a name="sql"></a>SQL

* Ajout d’une correspondance regex de HSM managé à SQL
* Mise à niveau de azure-mgmt-sql vers 0.26.0
* `az sql mi create/update` : Prise en charge de la configuration de maintenance dans les opérations d’instances managées
* Prise en charge des commandes de stratégie d’audit SQL Server DevOps

### <a name="storage"></a>Stockage

* Correctif #16079 : Objet blob public génère une erreur
* Disponibilité générale des informations de référence sur le routage de stockage
* Correctif #9158 : Impossible de générer une clé SAS opérationnelle à partir d’une stratégie
* Correctif #16489 : Mise à niveau d’azcopy vers 10.8.0
* `az storage account blob-service-properties` : Prise en charge de la version du service par défaut
* Correctif #16519 : azcopy reçoit des droits SAS inutiles (a des droits d’écriture, mais n’a besoin que de droits de lecture)

### <a name="synapse"></a>Synapse

* `az synapse workspace create ` : Ajout du paramètre `--key-identifier` pour prendre en charge la création d’un espace de travail à l’aide d’une clé gérée par le client.
* `az synapse workspace key` : Ajout d’applets de commande CRUD pour prendre en charge la gestion des clés dans l’espace de travail Synapse spécifié.
* `az synapse workspace managed-identity` : Ajout d’applets de commande pour prendre en charge les opérations CRUD sur l’identité managée avec le paramètre sql-access.
* `az synapse workspace` : Prise en charge de la protection contre l’exfiltration des données, ajout du paramètre `--allowed-tenant-ids`.
...
* `az acr helm` : Mise à jour de l’URL de dépréciation
* Ajout des changements logtemplate et systemtask pour ACR Tasks

### <a name="aks"></a>AKS

* Prise en charge de virtual-node avec aks create : `az aks create --enable-addons virtual-node`
* Ajout d’une option d’image de nœud seule pour CLI
* Désactivation par défaut prévue pour le module complémentaire kube-dashboard
* `az aks create/update` : Ajout de la prise en charge de LicenseType pour Windows
* Prise en charge de l’ajout d’un pool de nœuds Spot
* Respecter les noms des addons définis dans Azure CLI

### <a name="ams"></a>AMS

* Correctif 14687 : Mélange du groupe de ressources avec le nom de compte dans la commande "az ams streaming-endpoint show"

### <a name="app-config"></a>App Config

* Correction d’un bogue de test
* Prise en charge de l’authentification AAD pour les opérations de données

### <a name="app-service"></a>App Service

* `az functionapp deployment source config-zip` : Correction d’un problème où config-zip pouvait lever une exception en cas de réussite de la consommation Linux
* Résolution de bogue : Meilleurs messages d’erreur pour les commandes webapp
* `az appservice domain create, show-terms` : Ajout de la possibilité de créer un domaine de service d’application
* `az functionapp create` : Suppression de l’indicateur en préversion de Java 11 lors de la création d’une application de fonction
* [CHANGEMENT CASSANT] az webapp create, az webapp up - Mise à jour des runtimes d’application web disponibles
...
* Résolution du problème où il manquait la référence de l’image

### <a name="hdinsight"></a>HDInsight

* `az hdinsight create` : ajout d’informations de dépréciation pour l’argument --public-networrk-access-type et --outbound-public-network-access-type
* `az hdinsight create` : ajout d’informations de dépréciation pour l’argument `--public-networrk-access-type` et `--outbound-public-network-access-type`
* `az hdinsight create` : ajout du paramètre `--idbroker` pour prendre en charge le client visant à créer un cluster ESP avec un broker d’ID HDInsight

### <a name="iot-central"></a>IoT Central

* Suppression du module de commande 'az iotcentral' déprécié

### <a name="key-vault"></a>Key Vault

* Prise en charge de `--hsm-name` pour `az keyvault key encrypt/decrypt`

### <a name="lab"></a>Laboratoire

* Correction 14127 : `__init__()` prend 1 argument positionnel alors que 2 ont été donnés

### <a name="network"></a>Réseau

* `az network application-gateway ssl-cert show` : Ajout d’un exemple pour illustrer le format de certificat et récupérer des informations
* `az network application-gateway rule` : Prise en charge de l’option --priority
* `az network application-gateway create` : Correction du bogue où la création était impossible sans IP publique spécifiée
* `az network application-gateway waf-policy managed-rule rule-set add` : Présentation de l’erreur de serveur à l’utilisateur pour fournir un message indicatif plus intuitif.
* `az network application-gateway waf-policy managed-rule rule-set update` : Prise en charge du changement de version du type d’ensemble de règles.

### <a name="rdbms"></a>SGBDR

* Correction de bogue : az postgres flexible-server create : suppression de la version d’API codée en dur du client de réseau.

### <a name="role"></a>Role

* Correctif #15278 : `az role assignment list/delete` : Arguments de chaînes vides interdits

### <a name="sql"></a>SQL

* `az sql midb log-replay` : Prise en charge du service de relecture de journal sur une base de données managée
* Casse des caractères ignorée pour la valeur du paramètre de redondance du stockage de sauvegarde pour une instance managée
* [CHANGEMENT CASSANT] az sql db create : Ajout du paramètre --backup-storage-redundancy et ajout d’un avertissement pour unspecified bsr/bsr == Geo.

### <a name="sql-vm"></a>Machine virtuelle SQL

* `az sql vm show` : Ajout d’options de configuration à l’indicateur --expand

### <a name="storage"></a>Stockage

* [CHANGEMENT CASSANT] `az storage blob copy start` : Résolution du problème de format pour `--destination-if-modified-since` et `--destination-if-unmodified-since`
* [CHANGEMENT CASSANT] `az storage blob incremental-copy start` : Résolution du problème de format pour `--destination-if-modified-since` et `--destination-if-unmodified-since`
* `az storage fs` : Résolution du problème de chaîne de connexion
* `az storage share-rm` : Niveau d’accès de la version GA
* `az storage container-rm` : Ajout du nouveau groupe de commandes afin d’utiliser le fournisseur de ressources Microsoft.Storage pour les opérations de gestion de conteneurs.
...
* Correctif #14811 Ajout de la prise en charge du remplacement dockerignore

### <a name="aks"></a>AKS

* L’interface CLI doit tolérer une kubeconfig vide
* CORRECTIF #12871 : az aks enable-addons : L’exemple d’aide généré automatiquement est incorrect pour l’option virtual-node
* Suppression des actions du connecteur aci existantes
* Prise en charge de l’extension Azure Policy dans azure-cli
* Correction du problème de respect de la casse pour l’extension du tableau de bord AKS
* Mise à jour de mgmt-containerservice vers 9.4.0 et activation de l’API 09-01

### <a name="apim"></a>APIM

* Prise en charge des commandes d’entité product / productapi / namedValue et mise à jour de la version du SDK

### <a name="app-config"></a>App Config

* Prise en charge de l’activation/la désactivation de PublicNetworkAccess pour les magasins existants

### <a name="app-service"></a>App Service

* Ajout de la prise en charge du niveau tarifaire Premium v3
* Correctif #12653 : az webapp log config -- application-logging false ne le désactive pas
* Correctif #14684 : access-restriction remove par adresse IP ne fonctionne pas ; #13837-az webapp create - Exemple de RSgroups différents pour Plan et WebApp
* functionapp : Ajout de la prise en charge des gestionnaires personnalisés. Dépréciation de Powershell 6.2.
* functionapp : Correction du problème où le paramètre d’application était mal défini pour les images personnalisées Linux

### <a name="arm"></a>ARM

* `az deployment group/sub/mg/tenant what-if`: Affichage des changements de ressources « Ignorer » en dernier

### <a name="compute"></a>Compute

* Ajout d’un nouveau license_type dans vm create/update : RHEL_BYOS, SLES_BYOS
* Mise à niveau de la version d’API de disque avec la version 2020-06-30
* disk create: add --logical-sector-size, --tier
* mise à jour de disque : Prise en charge de --disk-iops-read-only, --disk-mbps-read-only, --max-shares
* Nouvelle commande disk-encryption-set list-associated-resources
* vm boot-diagnostics enable: --storage devient facultatif
* Nouvelle commande : vm boot-diagnostics get-boot-log-uris
* vm boot-diagnostics get-boot-log : prise en charge du stockage managé

### <a name="config"></a>Config

* Renommage de local-context en config param-persist

### <a name="cosmos-db"></a>Cosmos DB

* Prise en charge des API de migration pour la ressource de débit pour la fonctionnalité de mise à l’échelle automatique dans CosmosDB

### <a name="eventhub"></a>Event Hubs

Ajout des commandes Cluster et du paramètre trusted_service_access_enabled pour Networkruleset

### <a name="extension"></a>Extension

* `az extension add`: Ajout de l’option `--upgrade` pour mettre à jour l’extension si elle est déjà installée
* Activation de l’installation dynamique par défaut

### <a name="iot"></a>IoT

* Activation de la version TLS minimale sur IoT Hub Create

### <a name="iot-central"></a>IoT Central

* L’opération de suppression d’application est maintenant une opération durable

### <a name="iot-hub"></a>IoT Hub

* Dépréciation de la commande 'show-connection-string'

### <a name="key-vault"></a>Key Vault

* Gestion de la préversion publique de HSM
* Correction du problème où `--maxresults` ne prenait pas effet lors du listing des ressources ou des versions de ressource

### <a name="kusto"></a>Kusto

* Ajout d’un message de dépréciation

### <a name="monitor"></a>Monitor

* `az monitor log-analytics workspace linked-storage` : présentation d’un message d’erreur détaillé aux clients

### <a name="network"></a>Réseau

* `az network vnet subnet`: Prise en charge de --disable-private-endpoint-network-policies et de --disable-private-link-service-network-policies
* Correction du bogue de mise à jour de flow-log quand sa sous-propriété network_watcher_flow_analytics_configuration est None
* Mise à jour de la version de l’API avec 2020-06-01
* Prise en charge de --tcp-port-behavior lors de la configuration de TCP pour un Moniteur de connexion V2
* Prise en charge de types et de niveaux de couverture supplémentaires lors de la création d’un point de terminaison pour un Moniteur de connexion V2
* Prise en charge de --host-subnet pour créer VirtualHub en dessous comme VirtualRouter

### <a name="rdbms"></a>SGBDR

* Mises à jour du plan de gestion pour PostgreSQL et MySQL

### <a name="role"></a>Role

* `az role assignment create/update`: Prise en charge de `--description`, `--condition` et `--condition-version`
* `az ad app permission delete`: Prise en charge de `--api-permissions` pour supprimer un `ResourceAccess` spécifique

### <a name="service-fabric"></a>Service Fabric

* Ajout de commandes de type nœud et cluster managées

### <a name="sql"></a>SQL

* Mise à niveau d’azure-mgmt-sql vers 0.20.0
* Ajout d’un paramètre facultatif de redondance du stockage de sauvegarde à l’applet de commande MI create

### <a name="storage"></a>Stockage

* `az storage share-rm stats`: Obtention des octets d’utilisation des données stockées sur le partage.
* Version GA de la restauration à un instant dans le passé (PITR) d’objets blob de stockage
* `az storage blob query`: Prise en charge de l’accélération des requêtes de stockage Azure
* Prise en charge de la suppression réversible pour le partage de fichiers
* `az storage copy`: Ajout de la prise en charge des informations d’identification de compte et dépréciation de `--source-local-path`, `--destination-local-path` `--destination-account-name`
* `az storage account blob-service-properties update`: Ajout de la prise en charge de stratégie de conservation de conteneurs supprimés

### <a name="synapse"></a>Synapse

* Correction d’une faute de frappe dans l’exemple de création et de suppression d’attribution de rôles az synapse
...
## <a name="february-8-2021"></a>8 février 2021

> [!NOTE]
>
>  Des CHANGEMENTS CASSANTS sont introduits dans cette version.  Lisez attentivement toutes les notes de publication avant de procéder à l’installation.
>
> La version bêta ne garantit pas la qualité du produit. Ne l’utilisez pas dans votre environnement de production.

* Azure CLI bêta remplace en interne [ADAL](https://github.com/AzureAD/azure-activedirectory-library-for-python) par [Azure Identity](https://github.com/Azure/azure-sdk-for-python/tree/master/sdk/identity/azure-identity) et [MSAL](https://github.com/AzureAD/microsoft-authentication-library-for-python). Le cache de jetons ADAL existant (`~/.azure/accessToken.json`) est migré automatiquement vers le cache de jetons chiffré MSAL quand une commande nécessitant des informations d’identification est exécutée.

* Plusieurs modifications ont été apportées à `az login`.  (Pour plus d’informations, exécutez `az login --help`.)
  * [CHANGEMENT CASSANT] `~/.azure/accessToken.json` n’est plus créé après une connexion réussie. Pour obtenir un jeton d’accès, utilisez [`az account get-access-token`](https://docs.microsoft.com/cli/azure/account#az_account_get_access_token) à la place.
  * [CHANGEMENT CASSANT] L’argument `--use-cert-sn-issuer` n’est pas pris en charge.
  * Une fois que vous êtes connecté avec une identité managée, `clientId`, `objectId` et `resourceId` sont tous affichés.
  * Correctif n° 13188 : `az login` avec une identité managée indiquant le système affecté quand l’identité est affectée par l’utilisateur.

* [CHANGEMENT CASSANT] La méthode permettant d’ignorer la vérification SSL avec la variable d’environnement `ADAL_PYTHON_SSL_NO_VERIFY` a été supprimée. Consultez [Travailler derrière un proxy](https://docs.microsoft.com/cli/azure/use-cli-effectively#work-behind-a-proxy) pour approuver un certificat racine auto-signé.

La version bêta d’Azure CLI prend en charge toutes les commandes et reste synchronisée avec la version publiée actuelle.  

Pour obtenir des instructions d’installation, consultez [Installer la version bêta d’Azure CLI](install-azure-cli-beta.md). 

Si vous rencontrez des problèmes avec la version bêta, envoyez vos commentaires à l’équipe d’ingénieurs d’Azure CLI sur [GitHub](https://github.com/Azure/azure-cli/issues/new/choose).
